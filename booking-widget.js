      ...
      <div class="pane">
        ...
        <div class="service-list">
          ${SERVICES.map((s, i) => {
            ...
            return `<button class="choice service ${state.service === s.id ? "selected" : ""}" data-id="${s.id}" aria-pressed="${state.service === s.id}"><span class="service-icon">${["◉", "◌", "⌁"][i]}</span><span><b>${srv.name}</b><small>${srv.detail}</small></span><span class="duration">${srv.duration}</span></button>`;
          }).join("")}
        </div>
      </div>
    `;
