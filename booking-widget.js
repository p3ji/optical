(function () {
  ...
  const config = {
    apiBase: currentScript?.dataset.api || "",
  ...
  const css = `
    ...
    *,*:before,*:after{box-sizing:border-box}button,input,select{font:inherit}button{touch-action:manipulation} [hidden]{display:none!important}
    ...
    .side{background:var(--ink);color:white;padding:32px 28px;display:flex;flex-direction:column}.logo{display:flex;align-items:center;gap:10px;font-family:"Manrope",system-ui,sans-serif;font-weight:800;font-size:14px;letter-spacing:-.2px}.logo-icon{font-size:23px;color:var(--mint)}
    ...
    .main{min-width:0;display:flex;flex-direction:column;background:#fff}.topbar{min-height:72px;display:flex;align-items:center;justify-content:flex-end;padding:14px 25px;border-bottom:1px solid #edf0ec}.demo-pill{margin-right:auto;color:#54706a;background:#edf7f3;border-radius:999px;padding:7px 11px;font-size:10px;font-weight:800;letter-spacing:.8px;text-transform:uppercase}.close{width:44px;height:44px;border:0;border-radius:50%;background:#f4f4f0;color:var(--ink);cursor:pointer;font-size:23px;display:grid;place-items:center}.close:hover{background:#e8ece7}.close:focus-visible,.back:focus-visible,.next:focus-visible,.choice:focus-visible,.time:focus-visible,.day:focus-visible,input:focus-visible{outline:3px solid rgba(237,107,77,.3);outline-offset:2px}
    .content{padding:34px clamp(26px,5vw,56px) 20px;overflow:auto;flex:1;scrollbar-width:thin}.pane{animation:rise .28s ease both}@keyframes rise{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}.kicker{margin:0 0 8px;color:var(--coral);font-size:11px;font-weight:800;letter-spacing:1.6px;text-transform:uppercase}.title{font:800 clamp(28px,3vw,38px)/1.08 "Manrope",system-ui,sans-serif;letter-spacing:-1.4px;margin:0}.subtitle{margin:11px 0 25px;color:var(--muted);font-size:14px;line-height:1.55}
    ...
    .footer{min-height:82px;border-top:1px solid #edf0ec;display:flex;align-items:center;justify-content:space-between;padding:14px clamp(26px,5vw,56px);gap:12px}.back{border:0;background:transparent;color:#5e736d;font-size:12px;font-weight:800;cursor:pointer;padding:13px 0}.next{border:0;border-radius:999px;background:var(--ink);color:white;padding:14px 21px;min-height:47px;font-size:12px;font-weight:800;cursor:pointer;box-shadow:0 8px 18px rgba(16,46,43,.16);min-width:128px}.next:disabled{opacity:.35;cursor:not-allowed;box-shadow:none}.next:not(:disabled):hover{transform:translateY(-1px)}
  ...
  function datePane() {
    ...
    return `<div class="pane"><p class="kicker">Step 2 of 3</p><h1 class="title" id="booking-title">Pick a time that fits.</h1><p class="subtitle">Showing simulated live availability at ${esc(selectedBranch()?.name)}.</p><div class="date-layout"><section><div class="month-head"><button data-month="-1" aria-label="Previous month" ${monthOffset===0?"disabled":""}>‹</button><b>${month}</b><button data-month="1" aria-label="Next month">›</button></div><div class="week"><span>Sun</span><span>Mon</span><span>Tue</span><span>Wed</span><span>Thu</span><span>Fri</span><span>Sat</span></div><div class="days">${cells.map(d=>{if(!d)return`<span class="day empty"></span>`;const id=isoLocal(d);const disabled=id<today||d.getDay()===0;return`<button class="day ${id===today?"today":""} ${state.date===id?"selected":""}" data-date="${id}" ${disabled?"disabled":""} aria-label="${d.toDateString()}" aria-pressed="${state.date===id}">${d.getDate()}</button>`}).join("")}</div></section><section class="slot-panel"><div class="slot-title">${state.date?dateLabel():"Select a date"}</div><div class="slots">${state.date?slots.map(t=>`<button class="time ${state.time===t?"selected":""}" data-time="${t}" aria-pressed="${state.time===t}">${t}</button>`).join(""):`<p style="font-size:11px;color:#80908c;line-height:1.55;grid-column:1/-1">Available appointment times will appear here.</p>`}</div>${state.date?`<div class="availability"><span class="dot"></span> Calendar checked just now</div>`:""}</section></div></div>`;
  }
  function slotsFor(date) {
    if(!date)return[]; const seed=[...date+state.branch+state.service].reduce((a,c)=>a+c.charCodeAt(0),0); const all=["9:00 AM","9:30 AM","10:30 AM","11:00 AM","1:00 PM","1:30 PM","2:30 PM","3:00 PM","4:00 PM","4:30 PM"]; return all.filter((_,i)=>(i+seed)%4!==0).slice(0,8);
  }
  ...
  function bindPane(){
    ...
    $$("[data-month]").forEach(b=>b.addEventListener("click",()=>{monthOffset+=Number(b.dataset.month);state.date=null;state.time=null;render()}));$$("[data-date]").forEach(b=>b.addEventListener("click",()=>{state.date=b.dataset.date;state.time=null;render()}));$$("[data-time]").forEach(b=>b.addEventListener("click",()=>{state.time=b.dataset.time;render()}));
  ...
  async function continueFlow(){
    ...
    if(state.step===2&&config.apiBase){
      ...
      try{const res=await fetch(`${config.apiBase}/appointments`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({branch_id:state.branch,patient_name:state.name,patient_email:state.email,patient_phone:state.phone,service_type:selectedService()?.name,start_time:`${state.date} ${state.time}`})});if(!res.ok)throw new Error("Booking failed");}
      catch(e){toast("Live service unavailable — using demo handoff");}
    }
    state.step++;render();
  }
