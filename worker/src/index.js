    ...
    <tr>
      <td align="center">
        <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="max-width:600px;background:#ffffff;border-radius:24px;overflow:hidden;border:1px solid #e4eae4;box-shadow:0 12px 36px rgba(16,46,43,0.08)">
          ...
          <tr>
            <td style="background:#ed6b4d;color:#ffffff;padding:12px 24px;text-align:center;font-family:'Manrope',Arial,sans-serif;font-size:11px;font-weight:800;letter-spacing:1.5px;text-transform:uppercase">
              ⚠️ DEMO NOTICE: This is a prototype demonstration — Not a real medical appointment
            </td>
          </tr>
          <tr>
                    ...
                    <table cellpadding="0" cellspacing="0" role="presentation">
                      <tr>
                        ...
                        <td style="vertical-align:middle">
                          ...
                          <span style="display:block;font-size:11px;color:#9fd8c8;letter-spacing:0.5px;font-weight:500">Ottawa's Neighbourhood Opticians (Demo)</span>
                        </td>
                      </tr>
                    </table>
                  </td>
                  <td align="right" style="vertical-align:middle">
                    <span style="display:inline-block;background:rgba(159,216,200,0.16);border:1px solid rgba(159,216,200,0.3);color:#9fd8c8;font-size:10px;font-weight:800;letter-spacing:1.5px;padding:6px 12px;border-radius:999px;text-transform:uppercase">Demo Confirmed</span>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td style="padding:36px 32px 28px">
              <!-- Demo Notice Callout Box -->
              <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background:#fff3ee;border:1px solid #fedcd2;border-radius:14px;margin-bottom:24px">
                <tr>
                  <td style="padding:14px 18px;font-size:12px;color:#9b3820;line-height:1.6">
                    <b style="letter-spacing:0.5px">PORTFOLIO DEMONSTRATION ONLY:</b> This email was automatically generated by the Chicco Optical booking system prototype. It is <b>not a real clinic</b>, and no actual medical appointment has been booked.
                  </td>
                </tr>
              </table>
              ...
              <table width="100%" cellpadding="0" cellspacing="0" role="presentation">
                <tr>
                  <td>
                    <p style="margin:0 0 8px;color:#ed6b4d;font-size:11px;font-weight:800;letter-spacing:2px;text-transform:uppercase">Appointment Confirmed (Demo)</p>
                    ...
                    <p style="margin:0 0 24px;color:#4b6763;font-size:14px;line-height:1.6">Your visit has been reserved in our demo clinic calendar. Please find your simulated appointment details and calendar link below.</p>
                  </td>
                </tr>
              </table>
            ...
            <td style="background:#102e2b;padding:24px 32px">
              <table width="100%" cellpadding="0" cellspacing="0" role="presentation">
                <tr>
                  ...
                  <td align="right" style="vertical-align:middle">
                    <span style="display:inline-block;background:rgba(237,107,77,0.2);border:1px solid rgba(237,107,77,0.4);color:#f8d9ca;font-size:10px;font-weight:800;letter-spacing:1.5px;padding:6px 12px;border-radius:999px;text-transform:uppercase">Demo Reminder</span>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td style="padding:36px 32px 28px">
              ...
              <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background:#fff3ee;border:1px solid #fedcd2;border-radius:14px;margin-bottom:24px">
                <tr>
                  <td style="padding:14px 18px;font-size:12px;color:#9b3820;line-height:1.6">
                    <b style="letter-spacing:0.5px">PORTFOLIO DEMONSTRATION ONLY:</b> This reminder was generated by the Chicco Optical booking system prototype. It is <b>not a real clinic</b>, and no actual medical appointment has been scheduled.
                  </td>
                </tr>
              </table>
              ...
              <table width="100%" cellpadding="0" cellspacing="0" role="presentation">
                <tr>
                  <td>
                    <p style="margin:0 0 8px;color:#ed6b4d;font-size:11px;font-weight:800;letter-spacing:2px;text-transform:uppercase">Upcoming Appointment Reminder (Demo)</p>
                    ...
                    <p style="margin:0 0 24px;color:#4b6763;font-size:14px;line-height:1.6">This is a friendly reminder of your appointment tomorrow at Chicco Optical. Please review your simulated visit details below.</p>
                  </td>
                </tr>
              </table>
...
export async function sendConfirmation(env, booking, links) {
  const fromAddress = env.BOOKING_FROM_EMAIL || "Demo Optometry <bookings@peji.ca>";
  const coordinator = env.COORDINATOR_EMAIL || "pejisystems@gmail.com";
  const service = SERVICES[booking.service_type] || { name: "Eye Exam" };
  const payload = {
    from: fromAddress,
    to: [booking.patient_email],
    bcc: [coordinator],
    reply_to: coordinator,
    subject: `[DEMO] Confirmed: ${service.name} at Chicco Optical`,
  ...
  if (!response.ok && String(result.message || "").toLowerCase().includes("not verified") && booking.patient_email === coordinator) {
    ...
    payload.from = "Demo Optometry <onboarding@resend.dev>";
  ...
  if (!response.ok) {
    console.error("Resend API rejection for confirmation:", JSON.stringify(result));
...
export async function sendReminderEmail(env, booking, links) {
  ...
  const payload = {
    ...
    bcc: [coordinator],
    reply_to: coordinator,
    subject: `[DEMO] Reminder: Tomorrow's ${service.name} at ${formattedTime} — Chicco Optical`,
